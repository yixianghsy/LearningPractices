      ...
      <div class="container clearfix">
        ...
        <div class="content">
          ...
          <div class="item-version clearfix">
           ...
           <div class="phone fr"  v-for="(item ,index ) of product.skuStockList" :key="index"    :class="{'checked':clicked==index}"   :skuId="item.id" @click="handlerSkuid(item.id,index)" >{{item.sp1 + item.sp2}}</div>
  ...
  methods:{
    ...
    addCart(){
       if(this.skuId==null || this.skuId==undefined|| this.skuId==''){
          this.$message.warning('请选择一件版本/款式');
          return;
       }
      this.axios.post('/cart/add',{
        ...
        quantity:1
          ...
          this.$message.success('添加购物车成功，请到购物车中查看');
      });
       
    },
    handlerSkuid(skuId,index){
        this.clicked = index;
        this.skuId=skuId;
